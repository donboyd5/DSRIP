...
view <- readRDS(paste0(dbox, "DSRIP TSA_CA Team Info/Phil's files/shiny/V_mapp_pps_msr_ann_wide.rds"))
glimpse(view)
...
pprres <- pprres %>%
  group_by(PPS_ID) %>%
  mutate(change2018PPRRES = ifelse(PER_END_DT == "2014-06-30 00:00:00",
                                   PPRRES_MSR_RESULT- PPRRES_MSR_RESULT[PER_END_DT == "2018-06-30 00:00:00"],
                                   0))
...
pprres <- pprres %>%
  group_by(PPS_ID) %>% 
  mutate(pop2018 = ifelse(PER_END_DT == "2014-06-30 00:00:00",
                          PPRRES_MSR_DEN[PER_END_DT == "2018-06-30 00:00:00"], ""))

pprres <- pprres %>%
  group_by(PPS_ID) %>%
  mutate(tot2018 = ifelse(PER_END_DT == "2014-06-30 00:00:00",
                          PPRRES_MSR_NUM[PER_END_DT == "2018-06-30 00:00:00"],
                          ""))
...
PPRRESchangepop <- PPRRESchangepop %>%
  mutate(ChangePop = pop2018 - PPRRES_MSR_DEN, Changetot = tot2018-PPRRES_MSR_NUM)

###Create plot

PPRRESchangepop %>% 
  ggplot(aes(x= pop2018, y = change2018PPRRES)) + 
  geom_point(aes(size = pop2018)) + 
  ggtitle("Phil Change in PPRRES from 2014 to 2018 by 2018 population, weighted by population") + 
  xlab("Population 2018") +
  ylab("Change in PPRRES")+geom_smooth(method="lm")
...
meas <- "PPVRES"
(mnames <- paste0(meas, c("_MSR_RESULT", "_MSR_DEN", "_MSR_NUM")))
my2014 <- "2014-06-30 00:00:00"
my2018 <- "2018-06-30 00:00:00"
df <- view %>%
  select(PER_END_DT, result=mnames[1], den=mnames[2], num=mnames[3], PPS_ID, PPS_NAME) %>%
  filter(PER_END_DT %in% c(my2014, my2018), PPS_ID != 51) %>%
  group_by(PPS_ID, PPS_NAME) %>%
  mutate(pop2014=den[PER_END_DT==my2014],
         pop2018=den[PER_END_DT==my2018],
         meas2014=result[PER_END_DT==my2014],
         meas2018=result[PER_END_DT==my2018],
         pop_change=pop2018 - pop2014,
         meas_change=meas2018 - meas2014) %>%
  filter(PER_END_DT==my2018) %>%
  select(-PER_END_DT, -starts_with(meas))

df %>% 
  ggplot(aes(x= pop2018, y = meas_change)) + 
  geom_point(aes(size = pop2018)) + 
  ggtitle(paste0("Alternative code:  Change in ", meas, " from 2014 to 2018 by 2018 population, weighted by population")) + 
  xlab("Population 2018") +
  ylab(paste0("Change in ", meas)) + 
