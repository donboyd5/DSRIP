...
pps3 <- bind_rows(ppsm2, state) %>%
  left_join(measures %>% select(MSR_RESULT_ID, UNIT_LBL, improve)) %>%
  mutate(denom=case_when(UNIT_LBL == "Per 1,000 Newborns" ~ MSR_DEN / 1000,
                         UNIT_LBL == "Per 100 Members" ~ MSR_DEN / 100,
                         UNIT_LBL == "Per 100,000 Members" ~ MSR_DEN / 100e3,
                         UNIT_LBL == "Percentage" ~ MSR_DEN / 100,
...
msr_use <- measures %>%
  filter(npps==25, improve != "unknown") %>%
...
regressions <- regdata %>%
  ...
  nest(data=-c(MSR_RESULT_ID, PPS_ID, PPS_NAME)) %>%
    ...
    ztest=map(fit, ztest))
b <- proc.time()
b - a # 17 seconds for 516 regressions
...
# now determine which regressions were significant and coefficients had the desired sign

gooddf <- results %>%
  mutate(good=case_when((improve == "increase") &
                          (dsrip_ma.pvalue <= pcut) &
                          (dsrip_ma.coeff > 0) ~ 1,
                        
                        (improve == "increase") &
                          (interaction_ma.pvalue <= pcut) &
                          (interaction_ma.coeff > 0) ~ 1,
...
This program ran a total of `r nrow(gooddf)` regressions, including `r length(unique(gooddf$MSR_RESULT_ID))` measures, `r length(setdiff(unique(gooddf$PPS_ID), 1))` PPS's, and the statewide weighted average. I only included measures that were available for all 25 PPS's, and excluded any combinations of measures and PPS's for which there were one or more missing observations.

Overall, DSRIP had a significant effect in `r sprintf("%2.1f", mean(gooddf$good) * 100)`% of these regressions. I considered the DSRIP effect significant if the DSRIP intercept or DSRIP slope or both were significantly different from 0 at the `r sprintf("%0.2f", pcut)` level and the relevant DSRIP coefficient(s) had the sign needed for improvement in the measure.
...
for(meas in mlist$MSR_RESULT_ID[1:5]){
  p <- f(meas)
  print(p)
  ggsave(here::here("results", paste0(meas, "_regression_facet.png")), plot=p, width=10, height=10, scale=1.25)
}
...
## Robustness check auto.arima

```{r reg_auto}

# auto.arima(df$MSR_RESULT, xreg=xvars_ma)
a <- proc.time()
regressions_auto <- regdata %>%
  # additional filtering if desired:
  # filter(PPS_ID %in% c(99)) %>% # , MSR_RESULT_ID %in% "CAPC12MRES"
  arrange(PPS_ID, MSR_RESULT_ID, time) %>%
  nest(data=-c(MSR_RESULT_ID, PPS_ID, PPS_NAME)) %>%
    mutate(
    fit = map(data, ~ auto.arima(.$MSR_RESULT, 
                            xreg = as.matrix(.[, c("time", "dsrip", "interaction")]))),
    tidied = map(fit, tidy),
    glanced = map(fit, glance),
    ztest=map(fit, ztest))
b <- proc.time()
b - a # about 2+ mins for 516 regressions
saveRDS(regressions_auto, here::here("results", "regressions_auto.rds"))

regressions_auto %>% ht

coeffs <- regressions_auto %>% 
  unnest(tidied) %>%
  filter(str_sub(term, 1, 2) != "ma") %>%
  select(PPS_ID, MSR_RESULT_ID, term, estimate) %>%
  pivot_wider(names_from = term, values_from = estimate)

pvalues <- regressions_auto %>% 
  unnest(ztest) %>%
  select(PPS_ID, MSR_RESULT_ID, term, p.value) %>%
  pivot_wider(names_from = term, values_from = p.value)

results <- regressions_auto %>%
  select(PPS_ID, PPS_NAME, MSR_RESULT_ID) %>%
  left_join(measures %>% select(MSR_RESULT_ID, MSR_RESULT_NAME, improve), by = "MSR_RESULT_ID") %>%
  left_join(coeffs, by = c("PPS_ID", "MSR_RESULT_ID")) %>%
  left_join(pvalues, by = c("PPS_ID", "MSR_RESULT_ID"), suffix = c(".coeff", ".pvalue"))
...
gooddf <- results %>%
  mutate(good=case_when((improve == "increase") &
                        ...
                        (improve == "increase") &
                          (interaction.pvalue <= pcut) &
                          (interaction.coeff > 0) ~ 1,
...
## Robustness - auto.arima Tables of results
```{r include=TRUE}
...
## Robustness check auto.arima ma
...
coeffs <- regressions_automa %>% 
  unnest(tidied) %>%
  filter(str_sub(term, 1, 2) != "ma") %>%
  select(PPS_ID, MSR_RESULT_ID, term, estimate) %>%
  pivot_wider(names_from = term, values_from = estimate)

pvalues <- regressions_automa %>% 
  unnest(ztest) %>%
  select(PPS_ID, MSR_RESULT_ID, term, p.value) %>%
  pivot_wider(names_from = term, values_from = p.value)

results <- regressions_automa %>%
  select(PPS_ID, PPS_NAME, MSR_RESULT_ID) %>%
  left_join(measures %>% select(MSR_RESULT_ID, MSR_RESULT_NAME, improve), by = "MSR_RESULT_ID") %>%
  left_join(coeffs, by = c("PPS_ID", "MSR_RESULT_ID")) %>%
  left_join(pvalues, by = c("PPS_ID", "MSR_RESULT_ID"), suffix = c(".coeff", ".pvalue"))
...
# alternative definition of good -- slope only
gooddf <- results %>%
...
## Robustness - auto.arima ma Tables of results
```{r include=TRUE}
...
## Robustness auto.arima ma Facet plots for the 5 measures that had the greatest percentage of PPS's for which DSRIP was significant and indicated improvement
```{r automa_meas_regplot, fig.height=12, fig.width=12}
# get the measures we want
mlist <- gooddf %>%
  group_by(MSR_RESULT_ID, MSR_RESULT_NAME) %>%
  summarise(nregressions=n(), pct_signif=mean(good) * 100) %>%
...
for(meas in mlist$MSR_RESULT_ID[1:5]){
  p <- f(meas)
  print(p)
  ggsave(here::here("results", paste0(meas, "_automa_regression_facet.png")), plot=p, width=10, height=10, scale=1.25)
}
...
## Robustness automa Facet plots for the 5 PPS's that had the greatest percentage of measures for which DSRIP was significant and indicated improvement
```{r automa_pps_regplot, fig.height=12, fig.width=12}
# get the PPSs we want
plist <- gooddf %>%
  group_by(PPS_ID, PPS_NAME) %>%
  summarise(nregressions=n(), pct_signif=mean(good) * 100) %>%
...
g <- function(ppsid) {
  ...
  p <- ppsdf %>% 
    ...
    geom_vline(xintercept = 12, colour="blue", linetype="dashed") +
    theme_bw() +
    facet_wrap(~mname, ncol=4, scales = "free") +
    theme(legend.position = "none") +
    theme(axis.text.x=element_text(angle=90, vjust=0.5)) +
    ggtitle(gname,
            subtitle=paste0("Selected measures with significant DSRIP effect (slope or intercept) at ", sprintf("%.2f", pcut),
...
sw <- which(plist$PPS_ID==99)

for(ppsid in plist$PPS_ID[unique(c(1:5, sw))]){
  p <- g(ppsid)
  print(p)
  ggsave(here::here("results", paste0("pps_", ppsid, "_automa_regression_facet.png")), plot=p, width=10, height=10, scale=1.25)
}
