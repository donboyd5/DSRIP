...
# Introduction
This program automates Interrupted Time Series regression on DSRIP monthly data. 

The DOH data create many problems. The DSRIP measures are 12-month moving averages and have many shifts in levels up and down. This program addresses these as follows:

* It uses moving averages of the time and dummy variables on the right hand side.
* It addressses correlation in the error structure, generally through use of arima. 

It does not yet address level shifts in the data. A separate program identifies these shifts. It may be best to exclude from the ITS analysis any outcome measures that have significant shifts.
...
ppsnames <- chrisdf %>% 
  select(PPS_ID, PPS_NAME) %>% 
  unique %>% 
  mutate(idname=paste0(str_pad(PPS_ID, 2, side="left", "0"), " ", PPS_NAME))

saveRDS(ppsnames, paste0(don_dir, "ppsnames.rds"))
...
count(ppsall_ma %>% filter(rownum==1), PER_END_DT)  
...
# START OF ITS ANALYSIS -- with combined PPS/Statewide data
...
# results below are for msrid <- "AMRRES", ppsnum <- 99
...
# (Intercept) 57.96999    0.36505 158.801  < 2e-16 ***
# time        -0.21807    0.05163  -4.224 9.48e-05 ***
# dsrip        1.29379    0.42296   3.059  0.00348 ** 
# interaction  0.27899    0.05228   5.336 2.01e-06 ***
lmtest::coeftest(lm1)
# t test of coefficients:
# 
#              Estimate Std. Error  t value  Pr(>|t|)    
# (Intercept) 57.969992   0.365047 158.8012 < 2.2e-16 ***
# time        -0.218069   0.051626  -4.2241 9.477e-05 ***
# dsrip        1.293793   0.422960   3.0589   0.00348 ** 
# interaction  0.278986   0.052283   5.3360 2.013e-06 ***


# same model using arima call, same coefficients, dsrip impact big and signif
# notice that se is not identical to lm! but it matches lmtest ztest
mod1 <- arima(df$MSR_RESULT, xreg=xvars, order=c(0, 0, 0)) 
summary(mod1)
# Coefficients:
#       intercept     time   dsrip  interaction
#          57.970  -0.2181  1.2938       0.2790
# s.e.      0.352   0.0498  0.4078       0.0504
lmtest::coeftest(mod1)
# z test of coefficients:
#              Estimate Std. Error  z value  Pr(>|z|)    
# intercept   57.969992   0.352006 164.6846 < 2.2e-16 ***
# time        -0.218069   0.049781  -4.3805 1.184e-05 ***
# dsrip        1.293793   0.407849   3.1722  0.001513 ** 
# interaction  0.278986   0.050416   5.5337 3.135e-08 ***


# add ar1, dsrip smaller and not signif; se matches lmtest z
mod2 <- arima(df$MSR_RESULT, xreg=xvars, order=c(1, 0, 0)) 
summary(mod2)
# Coefficients:
#          ar1  intercept     time   dsrip  interaction
#       0.7931    58.7884  -0.1428  0.0886       0.2146
# s.e.  0.0823     0.5683   0.0744  0.4711       0.0835
lmtest::coeftest(mod2)
# z test of coefficients:
# 
#              Estimate Std. Error  z value Pr(>|z|)    
# ar1          0.793103   0.082287   9.6383  < 2e-16 ***
# intercept   58.788423   0.568293 103.4473  < 2e-16 ***
# time        -0.142795   0.074357  -1.9204  0.05481 .  
# dsrip        0.088596   0.471078   0.1881  0.85082    
# interaction  0.214614   0.083491   2.5705  0.01016 *  


# ar1 with ma variables, dsrip impact bigger but not signif
mod3 <- arima(df$MSR_RESULT, xreg=xvars_ma, order=c(1, 0, 0))
summary(mod3)
# Coefficients:
#          ar1  intercept  time_ma  dsrip_ma  interaction_ma
#       0.5448    56.7993  -0.2132    3.9887          0.2287
# s.e.  0.1090     0.4804   0.0415    0.6390          0.0411
lmtest::coeftest(mod3) # se's match


# auto.arima of proper data
mod4 <- auto.arima(df$MSR_RESULT, xreg=xvars_ma)
summary(mod4)
# Coefficients:
#          ar1  intercept  time_ma  dsrip_ma  interaction_ma
#       0.5448    56.7993  -0.2132    3.9887          0.2287
# s.e.  0.1090     0.4804   0.0415    0.6390          0.0411
# sigma^2 estimated as 0.1694:  log likelihood=-27.84 AIC=67.68   AICc=69.36   BIC=79.94 
lmtest::coeftest(mod4) # se's match
# z test of coefficients:
#                 Estimate Std. Error  z value  Pr(>|z|)    
# ar1             0.544787   0.109044   4.9960 5.852e-07 ***
# intercept      56.799306   0.480447 118.2218 < 2.2e-16 ***
# time_ma        -0.213186   0.041493  -5.1379 2.778e-07 ***
# dsrip_ma        3.988659   0.638996   6.2421 4.318e-10 ***
# interaction_ma  0.228740   0.041136   5.5605 2.689e-08 ***

# proper model ma11, neg but not signif
# se's match lmtest z test and lmtest t test where we specify df but note that p values are different!
# MORE IMPORTANT: NOTE THAT se's on indep vars are MUCH larger than for mod4 -- why??
mod5 <- arima(df$MSR_RESULT, xreg=xvars_ma, order=c(0, 0, 11), fixed = c(rep(1, 11), NA, NA, NA, NA ))
summary(mod5)
# Coefficients:
...
# z test of coefficients:
#                Estimate Std. Error z value Pr(>|z|)    
# intercept      57.54741    2.04784 28.1016   <2e-16 ***
# time_ma        -0.15234    0.15446 -0.9863   0.3240    
# dsrip_ma        2.68067    2.60075  1.0307   0.3027    
# interaction_ma  0.19384    0.16434  1.1794   0.2382    
lmtest::coeftest(mod5, df=53)
# t test of coefficients:
#                Estimate Std. Error t value Pr(>|t|)    
# intercept      57.54741    2.04784 28.1016   <2e-16 ***
# time_ma        -0.15234    0.15446 -0.9863   0.3285    
# dsrip_ma        2.68067    2.60075  1.0307   0.3073    
# interaction_ma  0.19384    0.16434  1.1794   0.2435  
...
*  assumption checks
*  robustness checks - run lm equivalent
*  one-sided ztests?
*  slope-intercept graphs of coefficients
...
ztest <- function(mod, dgrf="nocalc"){
  # calculate the z test statistic (essentially the t) and its p.value
  # return a data frame
  ...
  if(dgrf == "nocalc") {
    ztest <- lmtest::coeftest(mod) 
      ...
      # calculate degrees of freedom - we must do this 
      # if some of the coefficients are restricted
      nfree_vars <- nrow(mod$var.coef) # only the free (unrestricted) variables will be in here
      dfreedom <- mod$nobs - nfree_vars
      ztest <- lmtest::coeftest(mod, df=dfreedom)
      }
...
ztest2 <- function(mod){
  # direct calculation of t statistic
  # calculate degrees of freedom - we must do this 
  # if some of the coefficients are restricted
  nfree_vars <- nrow(mod$var.coef) # only the free (unrestricted) variables will be in here
  dfreedom <- mod$nobs - nfree_vars
  ...
  ztdf <- ztest[, ] %>% 
    ...
    mutate(term=rownames(ztest)) %>%
    as_tibble() %>%
    mutate(degrees=dfreedom, 
           p.value.calc=2*(1-pt(z.value, degrees))) %>%
    select(term, everything())
  ztdf
}
...
# glimpse(results)
# results <- get_sig(results, shift="dsrip_ma", slope="interaction_ma", .01)
...
#   datatable(colnames=c("Number of PPS's (counting statewide total as a PPS)"="npps",
#                        "Number of regressions"="nregressions"),
#             rownames = FALSE,
#             options = list(ordering=FALSE)) %>%
#   formatStyle("Number of PPS's (counting statewide total as a PPS)", `text-align` = 'left')

nregs %>%
  janitor::adorn_totals() %>%
  kable(align=c("l", "r"),
        col.names=c("Number of PPS's (counting statewide total as a PPS)",
...
```{r results='asis'}
vars <- c("shift", "slope", "both", "none") # these are in the desired sort order
labs <- c("Intercept only", "Slope only", "Both intercept and slope", "Not significant")

results %>%
  group_by(sig_type) %>%
  summarise(n=n()) %>%
  mutate(sig_type=factor(sig_type, levels=vars, labels=labs)) %>%
  arrange(sig_type) %>%
  janitor::adorn_totals() %>%
  mutate(pct=n / n[sig_type=="Total"] * 100) %>%
  kable(caption="Number of regressions by type of significance",
        align=c("l", "r", "r"),
        digits=c(0, 0, 1),
        col.names=c("Type of significance",
                    "Number of regressions",
                    "Percent of total")) %>%
  kable_styling()
...
## Facet plots for measures that had the greatest percentage of PPS's for which DSRIP was significant and indicated improvement
...
for(meas in mlist$MSR_RESULT_ID[1:10]){
  p <- f(meas)
  print(p)
...
## Facet plots for PPS's that had the greatest percentage of measures for which DSRIP was significant and indicated improvement
...
g <- function(ppsid) {
  ...
  p <- pdata %>% 
    ...
    mutate(mname=paste0(MSR_RESULT_ID, ": ", msr_name(MSR_RESULT_ID)) %>% str_wrap(35)) %>%
...
Structure of the policy change:

*  ITS requires us to identify when the policy change happened. If it was phased in (or phased into the data) we need to be able to identify that. We should try to learn when and how PPS's implemented DSRIP, and what sort of lags there might be.
*  ITS is best with outcomes that can change quickly after the policy change. If that cannot happen (e.g., it may be hard with clinical measures), intermediate outcomes (e.g., access to care, advisement of patients) that can change quickly may be more appropriate.

Technical issues:

* linear trend or transformation to linear
...
*  Studies with few time points or with small expected effect sizes should be interpreted with caution as they may be underpowered
*  Power is also increased if the numbers of data points are equally distributed before and after the intervention, though this is often not practical



# Robustness checks and variants
...
regressions_mafixed <- regdata %>%
  ...
  nest(data=-c(MSR_RESULT_ID, MSR_RESULT_NAME, PPS_ID, PPS_NAME)) %>%
    ...
    tidied = map(fit, tidy),
    glanced = map(fit, glance),
    ztest=map(fit, ztest, dgrf="calc"))
b <- proc.time()
b - a # 40 seconds for 605 regressions
...
```{r its_ols, include=FALSE}
# run ols regressions with untransformed data, just for comparison
a <- proc.time()
xvars <- c("time", "dsrip", "interaction")
regressions_ols <- regdata %>%
  # additional filtering if desired:
  # filter(PPS_ID %in% (c(1:99)) %>%
  arrange(PPS_ID, MSR_RESULT_ID, time) %>%
  nest(data=-c(MSR_RESULT_ID, MSR_RESULT_NAME, PPS_ID, PPS_NAME)) %>%
...
saveRDS(regressions_ols, here::here("results", "regressions_ols.rds"))

regressions_ols <- readRDS(here::here("results", "regressions_ols.rds"))
regressions_ols %>% ht

```


## Comparison to OLS model with untransformed data

```{r include=FALSE}
regressions_ols <- readRDS(here::here("results", "regressions_ols.rds"))
glimpse(regressions_ols)

rlist_ols <- reg_results(regressions_ols)

results_ols <- rlist_ols$results
glimpse(results_ols)
...
results_ols <- get_sig(results_ols, shift="dsrip", slope="interaction", pvalue.cut=pcut)
count(results_ols, sig_type)
mean(results_ols$good)
...
## Comparison to arima model with ma data and 11 mixed ma error coefficients

```{r include=FALSE}
regressions_mafixed <- readRDS(here::here("results", "regressions_mafixed.rds"))
glimpse(regressions_mafixed)

rlist_mafixed <- reg_results(regressions_mafixed)

results_mafixed <- rlist_mafixed$results
glimpse(results_mafixed)
...
results_mafixed <- get_sig(results_mafixed, shift="dsrip_ma", slope="interaction_ma", pvalue.cut=pcut)
count(results_mafixed, sig_type)
mean(results_mafixed$good)
