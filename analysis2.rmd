...
mdates <- chris_m %>%
  group_by(MSR_RESULT_ID, MSR_RESULT_NAME, MSR_AS_OF_DT) %>%
  summarise(nobs=n()) %>%
  group_by(MSR_RESULT_ID, MSR_RESULT_NAME) %>%
  mutate(n_asof=n()) %>%
  ungroup
mdates %>% filter(n_asof > 1)


count(chris_m, MSR_RESULT_ID, MSR_RESULT_NAME, UNIT_LBL)
```
...
tmp <- ppsm2 %>%
  mutate(msr_calc=MSR_NUM / MSR_DEN * 100, # discovered that it must be x 100 
         diff=msr_calc - MSR_RESULT) %>%
  filter(diff!=0)
quantile(tmp$diff, na.rm=TRUE)

# apparently some of the measures are off by a factor of 100 - which ones??
