...
ar1_free <- arima(df$MSR_RESULT, xreg=xvars_mab, order=c(1, 0, 0))
summary(ar1_free)

ma11_fixed <- arima(df$MSR_RESULT, xreg=xvars_mab, order=c(0, 0, 11), fixed = c(rep(1, 11), NA, NA, NA, NA, NA))
summary(ma11_fixed)

ma11_fixed2 <- arima(df$MSR_RESULT, xreg=xvars_mab2, order=c(0, 0, 11), fixed = c(rep(1, 11), NA, NA, NA, NA, NA, NA))
summary(ma11_fixed2)

# compare to modyy2 which is same as ma11_fixed but without the break dummy
summary(modyy2)
