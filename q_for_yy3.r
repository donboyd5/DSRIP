...
data <- data %>%
  mutate(breakdum=ifelse(time_ma>0, 1, 0),
         breakdum2=ifelse(time_ma>18, 1, 0))
data

xvars_ma <- as.matrix(data[, c("time_ma", "dsrip_ma", "interaction_ma")])
xvars_mab <- as.matrix(data[, c("time_ma", "dsrip_ma", "interaction_ma", "breakdum")])
xvars_mab2 <- as.matrix(data[, c("time_ma", "dsrip_ma", "interaction_ma", "breakdum", "breakdum2")])
...
#  2) the rise in aic
modyy2 <- arima(data$MSR_RESULT, xreg=xvars_ma, order=c(0, 0, 11), fixed = c(rep(1, 11), NA, NA, NA, NA ))
summary(modyy2)
# Coefficients:
...
modyy3 <- arima(data$MSR_RESULT, xreg=xvars_ma, order=c(0, 0, 11))
summary(modyy3)
# Coefficients:
#          ma1     ma2      ma3      ma4      ma5      ma6      ma7      ma8      ma9    ma10    ma11  intercept  time_ma  dsrip_ma  interaction_ma
...
modyy4 <- arima(data$MSR_RESULT, xreg=xvars_ma, order=c(1, 0, 11), fixed = c(NA, rep(1, 11), NA, NA, NA, NA))
summary(modyy4)


# now re-estimate with a dummy for the major break and compare ar1-free to ma11-fixed
ma11_fixed <- arima(data$MSR_RESULT, xreg=xvars_mab, order=c(0, 0, 11), fixed = c(rep(1, 11), NA, NA, NA, NA, NA))
summary(ma11_fixed)

ma11_fixed2 <- arima(data$MSR_RESULT, xreg=xvars_mab2, order=c(0, 0, 11), fixed = c(rep(1, 11), NA, NA, NA, NA, NA, NA))
summary(ma11_fixed2)

# compare to modyy2 which is same as ma11_fixed but without the break dummies
summary(modyy2)


summary(modyy1)

ar1_free <- arima(data$MSR_RESULT, xreg=xvars_mab, order=c(1, 0, 0))
summary(ar1_free)

ar1_free2 <- arima(data$MSR_RESULT, xreg=xvars_mab2, order=c(1, 0, 0))
summary(ar1_free2)

auto.arima(data$MSR_RESULT, xreg = xvars_mab2)


data %>% ggplot(aes(time_ma, MSR_RESULT)) + geom_line() + geom_point()
